...
import dumb.Section.SusSection;
import haxe.Json;
...
class Song
{
	...
	public var notes:Array<SusSection>;
	...
	private static function onLoadJson(songJson) // Convert old charts to newest format
	{
		...
		if(songJson.events == null)
		{
			...
			{
				var sec:SusSection = songJson.notes[secNum];
	...
	public static function loadFromJson(jsonInput:String, ?folder:String):SusSong
	{
		var rawJson = null;
		
		var formattedFolder:String = 'Assets/JsonData/Songs/$folder/';
		...
		var songJson:SusSong = parseJSONshit(rawJson);
		//if(jsonInput != 'events') StageData.loadDirectory(songJson);
		onLoadJson(songJson);
		return songJson;
	}

	public static function parseJSONshit(rawJson:String):SusSong
	{
		var swagShit:SusSong = cast Json.parse(rawJson).song;
